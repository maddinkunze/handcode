...
class Hand(object):
    ...
    def __init__(self):
        ...
        self.nn = rnn(
            log_dir=os.path.join(package_directory, 'logs'),
            checkpoint_dir=os.path.join(package_directory, 'checkpoints'),
            prediction_dir=os.path.join(package_directory, 'predictions'),
    ...
    def _sample(self, lines, biases=None, styles=None):
        ...
        if styles is not None:
            for i, (cs, style) in enumerate(zip(lines, styles)):
                x_p = np.load('{}/styles/style-{}-strokes.npy'.format(package_directory, style))
                c_p = np.load('{}/styles/style-{}-chars.npy'.format(package_directory, style)).tostring().decode('utf-8')
    ...
    def _draw(self, strokes, lines, filename, stroke_colors=None, stroke_widths=None):
        ...
        for offsets, line, color, width in zip(strokes, lines, stroke_colors, stroke_widths):
            ...
            #strokes[:, 0] += (view_width - strokes[:, 0].max()) / 2 # center text, we dont want that
